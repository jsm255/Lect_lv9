...
class SnakePanel2 extends JPanel implements KeyListener, ActionListener{
	...
	public SnakePanel2() {
		...
		// requestFocusInWindow()  <= 이 친구를 페인트컴포넌트에 넣어주자
	}
	...
	private void setButton() {
		...
		this.reset.setBounds(700-200, y+100, 150, 50);
	...
	public void keyPressed(KeyEvent e) {
		System.out.println(e.getKeyCode());	// e.getKeyCode() = 입력된 키자판의 값을 가져옴
		
		if(e.getKeyCode() == e.VK_LEFT) this.dir = LEFT;
		else if(e.getKeyCode() == e.VK_DOWN) this.dir = DOWN;
		else if(e.getKeyCode() == e.VK_RIGHT) this.dir = RIGHT;
		else if(e.getKeyCode() == e.VK_UP) this.dir = UP;
	...
	protected void paintComponent(Graphics g) {
		...
		requestFocusInWindow(); // 창에 대한 관심을 다시 요청
		repaint();
	}
	...
	private void update() {
		...
		for(int i = 0; i<this.yx.length; i++) {
			if(yy == this.yx[i][0] && xx == this.yx[i][1]) bodyCheck = true;
		}
		if(bodyCheck) return;
		...
		for(int i = this.snake.length-1; i>0; i--) {
			...
			this.snake[i].setC(Color.blue);
			
			// 좌표 갱신
//			this.yx[i] = this.yx[i-1];		// 주소가 공유되어버렸음!
			int[] pair = new int[2];
		...
		this.snake[0] = new SnakeRect(t.getX(), t.getY(), t.getW(), t.getH(), Color.pink);
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() instanceof JButton) {
			...
			if(target == this.reset) {
				...
				if(target == this.btn[LEFT]) this.dir = LEFT;
				else if(target == this.btn[DOWN]) this.dir = DOWN;
				else if(target == this.btn[RIGHT]) this.dir = RIGHT;
				else if(target == this.btn[UP]) this.dir = UP;
			}
		}
	}
...
class SnakeFrame2 extends JFrame{
...
public class Ex풀이_스네이크 {

	public static void main(String[] args) {
		
		SnakeFrame2 sf = new SnakeFrame2();
