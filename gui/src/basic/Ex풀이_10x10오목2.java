...
class Omok2Panel extends JPanel implements MouseListener{
		...
		for(int i = 0; i<SIZE; i++) {
			for(int j = 0; j<SIZE; j++) {
				...
				if(r.getOwner() != 0) {
					...
					g.drawRoundRect(r.getX()+5, r.getY()+5, r.getW()-10, r.getH()-10,
							r.getW(), r.getH());
					g.setColor(r.getC());
					g.fillRoundRect(r.getX()+5, r.getY()+5, r.getW()-10, r.getH()-10,
							r.getW(), r.getH());
				}
			}
		}
	...
	private void checkWin() {
		...
		if(this.win != 0) {
			...
			OmokResult or = new OmokResult(String.format("p%d 승리!", this.win));
		}
	}
